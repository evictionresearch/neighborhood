...
### Multiple layers with a switcher
...
add `nt_layers_control()` — it shows **one layer at a time** and reveals only that
layer's legend. Here we put the neighborhood typology and the share of each
demographic group on one map:

```{r}
demos <- c(White = "pWhite", Black = "pBlack", Asian = "pAsian", Latine = "pLatine")

m <- nt_maplibre(balt) |>
  nt_add_choropleth(balt, "nt_conc", id = "Typology", legend_title = "Neighborhood type")
...
knitr::include_graphics("figures/map-layers.png")
```

The same pattern works for any set of measures — it is how the HPRM map toggles
its eviction-risk, displacement-risk, and composite layers (just give each its
own `id`). For a raw multi-select (checkbox) toggle instead of a one-at-a-time
switcher, drop to `mapgl::add_layers_control()`.
